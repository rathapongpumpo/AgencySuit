...
class DealController extends Controller
{
    ...
    public function store(DealRequest $request, Client $client, PlanService $planService, PostHogAnalytics $analytics): RedirectResponse
    {
        Gate::authorize('view', $client);
        ...
        $analytics->track($request, 'deal_created');
        if ($deal->stage === 'closed') {
            $analytics->track($request, 'deal_closed');
        }
    ...
    public function update(DealRequest $request, Deal $deal, PostHogAnalytics $analytics): RedirectResponse
    {
        Gate::authorize('update', $deal);
        ...
        $deal->update($data);
        if ($deal->getOriginal('stage') !== 'closed' && $deal->stage === 'closed') {
            $analytics->track($request, 'deal_closed');
        }
